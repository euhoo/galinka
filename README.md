...
<h3 id="driven">3.В компоненте, который использует данные из хранилища:</h3>  
    ...
    г) единожды для каждого компонента, использующего G добавь функцию, вызывающую перерендеринг страницы:
        ...
        ```  
        ВАЖНО! Добавляй эту функцию в том месте, которое вызывается лишь однажды за жизненный цикл компонента. Иначе при каждом срабатывании функции будет происходить добавление еще одной функции и каждый рендеринг будет увеличиваться на один раз.  
